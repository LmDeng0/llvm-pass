...
{
    //
    //是否从生成的数据流图（DFG）中删除不必要的叶节点。这些选项可以通过命令行参数进行设置
    ...
    struct DFGGenPass : public LoopPass
    {
        ...
        virtual bool runOnLoop(Loop *L, LPPassManager &LPM)
        {
            ...
            if(!(L->getBlocks().empty())) // if empty there are no sub loops
                return false;
